...
char toascii_frombinary(const char *s)
{
  ...
  while (*p != '\0' && p >= s)
  {
    if (*p != '0' && *p != '1')
    {
      throw new std::invalid_argument("invalid binary number");
    }
    if (*p-- == '1')
    {
      c += power;
...
std::string Base64toBinary(std::string base64)
{
  ...
  for (int i = 0; i < base64.length(); i++)
  {
    char chr = base64[i];
    if (chr >= 'A' && chr <= 'Z')
      binaryString.append(tobinary((chr - 'A'), 6));
    else if (chr >= 'a' && chr <= 'z')
      binaryString.append(tobinary((chr - 'a' + ('Z' - 'A') + 1), 6));
    else if (chr >= '0' && chr <= '9')
      binaryString.append(tobinary((chr - '0' + ('Z' - 'A') + ('z' - 'a') + 2), 6));
    else if (chr == '+')
      binaryString.append(tobinary(62, 6));
    else if (chr == '/')
      binaryString.append(tobinary(63, 6));
    else
...
void HexStringto4x4Array(std::string encryptedText, int destArray[4][4])
{
  ...
  copy4x4Array(text_Array, destArray);
}

void ExpandKey(int initialArray[4][4], int array[4][44])
{
  ...
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < 4; j++)
    {
      expandedKey[i][j] = initialArray[i][j];
    }
  }
...
// void printMxNArray(int row, int column, int array[4][])
// {
//   for (int i = 0; i < row; i++)
//   {
...
void XOR4x4Array(int array1[4][4], int array2[4][4], int destArray[4][4])
{
  for (int j = 0; j < 4; j++)
  {
    for (int k = 0; k < 4; k++)
    {
      destArray[j][k] = array1[j][k] ^ array2[j][k];
    }
  }
}

void EncryptAES128Bit(int initialArray[4][4], int expandedKeyArray[4][44], int encryptedArray[4][4])
{
  ...
  copy4x4Array(initialArray, text_Array);
...
void DecryptAES128BitArray(int encryptedArray[4][4], int expandedKeyArray[4][44], int decryptedArray[4][4])
{
  std::vector<int> temp_Vec;
  std::vector<int> tempXorVec;
  int text_Array[4][4];
  ...
  copy4x4Array(encryptedArray, text_Array);
